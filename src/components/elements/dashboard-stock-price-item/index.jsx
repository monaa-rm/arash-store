...
const DashboardStockPriceItem = ({ item }) => {
  const [openBox, setOpenBox] = useState(false);
  const [productStock, setProductStock] = useState(item?.inStock || 0);
  const [productCost, setProductCost] = useState(item?.price || 0);
  useEffect(() => {
    const handleClick = (event) => {
      if (!event.target.closest(`#DashboardStockPriceItem${item?.id}`)) {
        setOpenBox(false);
      }
    };
  ...
  return (
    <div
      id={`DashboardStockPriceItem${item?.id}`}
