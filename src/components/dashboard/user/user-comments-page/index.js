  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/comment/getUserComments?page=${currentPage}&limit=${commentPerPage}`
      );
      ...
      {loading ? (
        ...
        <div className="w-full flex flex-col gap-3 py-2 md:p-4">
          {comments?.length === 0 ? (
            ...
            comments.map((item) => <CommentItem key={item?._id} item={item} comments={comments} setComments={setComments} />)
          ) : (
