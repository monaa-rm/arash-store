...
const menuData = [
  ...
  { title: "افزودن محصول", link: "new-product" },
  { title: "ویرایش/حذف محصول", link: "edit-product" },
  { title: "محصولات پیشنهادی", link: "suggest-product" },
  ...
  { title: "وبلاگ", link: false, iconname: "blog" },
...
const DashboardAdminLayout = ({ children }) => {
  ...
  useEffect(() => {
    const handleClick = (event) => {
      if (!event.target.closest("#admindashboardlist")) {
        setShowList(false);
      }
    };
  ...
  return (
      ...
      >
        <div className="flex flex-col gap-0.5 py-3 px-4">
          {menuData?.map((item, i) => (
            <div
              onClick={() => {
                item?.link ? dispatch(setDashboardActiveItem(item)) : null;
                setShowList(false);
              }}
              key={i}
   ...
   active:scale-90 before:absolute before:top-0 before:-left-full before:w-full before:h-full
    ...
    before:duration-500 before:ease-in-out before:z-[-1] before:rounded-tl-full before:rounded-bl-full  hover:before:left-0"
              >
                <Link
                  href={`/dashboard/admin/${dashboardActiveItem?.link}`}
                  className="w-full h-full flex justify-center items-center"
                >
                  {dashboardActiveItem?.title}
                </Link>
